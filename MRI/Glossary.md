...
### DICOM File
A file in the DICOM format. It stores one or multiple images or spectra ([Processed Data](#processed-data)) coming from a scan, and a header which contains the [Metadata](#metadata) organized as tags following the DICOM standard. When multiple images or spectra are stored in the same [DICOM File](#dicom-file), they belong to the same [Series](#Series). The term has been adapted from Varma (2012)[^1].
...
### Series
A set of one or more images or spectra (i.e. [Processed Data](#processed-data)) generated during [Measurements](#measurement) along the z-axis of a [Sample](#sample), collected as part of the same [Study](#study) and stored in one or multiple [DICOM Files](#dicom-file). The term has been adopted from DICOM[^2].
